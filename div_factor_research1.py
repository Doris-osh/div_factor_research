...
def factor_zscore_neutral(factor):
    ...
    mv_data = globals()['mv'].apply(np.log)
...
factor_to_test = ['div_12m','pe_ttm','mv']
...
single_factor_test(factor_to_test)

'''def dual_factor_cross_test(factor1, factor2, bins_n):
    labels = ['第' + str(i + 1) + '分位' for i in range(bins_n)]
    for trade_date in globals()[factor1 + '_t'].columns[1:]:
    globals()[f'{factor1}&{factor2}' + '_t_groups'] = pd.DataFrame(index=globals()[factor1 + '_t'].index)
    globals()[f'{factor1}&{factor2}' + '_groups_return'] = pd.DataFrame(index=globals()[factor1 + '_t'].columns,columns=labels + ['基准组合'])
    globals()[f'{factor1}&{factor2}' + '_groups_value'] = pd.DataFrame(index=globals()[factor1 + '_t'].columns,columns=labels + ['基准组合'])
    globals()[f'{factor1}&{factor2}' + '_groups_value'].iloc[0] = 1.0000
    pre_value = globals()[factor1 + '_groups_value'].iloc[0]
    for trade_date in globals()[factor1+'_t'].columns[1:]:
        globals()[factor1 + '_t_groups'][trade_date] = pd.qcut(globals()[factor1+'_t'][trade_date],bins_n,labels=labels)
        for bin in labels:
            stock_for_bin = list(globals()[factor + '_t_groups'][globals()[factor + '_t_groups'][trade_date]==bin].index)
            globals()[factor + '_groups_return'].loc[trade_date,bin] = globals()['stock_return'].loc[trade_date,stock_for_bin].mean()
            globals()[factor + '_groups_value'].loc[trade_date,bin] = pre_value[bin] * (1 + globals()[factor + '_groups_return'].loc[trade_date,bin])
